...
def _parse_pdf_records(pdf_path: str, llm_cfg: dict,
    ...
    raw_records = extract_pdf_records(
        ...
        extra_exclusion_note=(
            ...
            "commercial land sales exist). "
            "EXTRACT from a table whose title/columns indicate a LAND SALE or "
            "GOVERNMENT LAND SALES (GLS) tender — e.g. a title like 'Successful "
            "Tender' or columns like 'Successful Tenderer', 'Date of Award', or "
            "'psf ppr' — or development-site transactions. "
            "SKIP the whole table when its title/columns indicate the sale of "
...
- SCOPE — this is a LAND SALES / GLS / SITE TENDER analysis. Decide what the table is
  from its TITLE and COLUMN HEADERS only — never from the property use/sector of
  individual rows (both residential AND commercial land sales exist). EXTRACT when the
  title/columns indicate a LAND SALE or Government Land Sales (GLS) tender — e.g. title
  'Successful Tender' or columns 'Successful Tenderer' / 'Date of Award' / 'psf ppr' —
  or development-site transactions. SKIP the whole table (return []) when the
  title/columns indicate the sale of completed buildings / investment assets (a
  Buyer/Purchaser column with built-property names) or Leasing / Rental activity.
...
# (shared with sales/rent) — these are thin per-comp-type wrappers over it.
#
# exact_km=0.02 (20m): near-identical coordinates force a merge even if price
# disagrees — safe for land, since a site is normally sold/tendered once per
# window (unlike rent, which never sets this — see scan_input_rent_comps.py).
...
def _dedup_cross_source(records: list, threshold_km: float = 0.05,
    ...
    return _dedup_cross_source_shared(
        records, value_fields=("price_sgd_m",),
        name_keys=("site_name", "property_name"), name_min_overlap=0.9,
...
def _flag_cross_source_conflicts(records: list, threshold_km: float = 0.2,
    ...
    return _flag_cross_source_conflicts_shared(
        records, value_fields=(("price_sgd_m", "price"), ("price_psf_ppr", "psf ppr")),
        name_keys=("site_name", "property_name"),
...
def _geocode_comps(records: list, google_key: str,
    ...
    for r in records:
        ...
        # Address-vs-name policy shared with sales/rent (resolve_geocode_queries):
        # a real street address is geocoded alone; otherwise geocode by name with
        # the submarket as a backend-only hint.
...
def run(config_path: str = "configs/deal_config.json",
    ...
    # map_cfg, so a deal config missing/blank on "mapbox.token" silently skipped
    # the map for LAND (while sales still worked via the shared_settings fallback).
    _ss_path = Path(__file__).parent.parent / "configs" / "shared_settings.json"
    try:
        google_key = json.loads(_ss_path.read_text(encoding="utf-8")).get("google_maps_key", "")
    except Exception:
        google_key = ""
    if not google_key:
    ...
    # Prefer an explicit code, else derive it from country_name (no Singapore hardcode)
    # so geocoding restricts to the right country.
    ...
    if google_key:
        ...
        print(f"\n[1/5] No Google Maps key — geocoding skipped (comps ranked by order)")
    ...
    if google_key and s_lon is not None:
        ...
        records = _geocode_comps(records, google_key, country_code, country_name,
                                  s_lon, s_lat)

        # Flag same-site cross-source value disagreements BEFORE merging, so the
        # (deliberately unmerged) conflicting rows carry their review note.
        records = _flag_cross_source_conflicts(records)

        _before_dedup = len(records)
        records = _dedup_cross_source(records)
        if len(records) < _before_dedup:
            print(f"      Cross-source dedup: {_before_dedup} → {len(records)} "
            ...
            for sr in _saved_records:
                ...
                if _meta:
                    for _fld in ("lon", "lat", "map_marker",
                                 "_geo_provider", "_geo_note", "_geo_suspect"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--map", action="store_true",
                        help="(no-op — map is generated by default; kept for old scripts)")
    parser.add_argument("--no-map", action="store_true",
                        help="Skip generating the Mapbox map PNG")
    ...
    run(args.config, generate_map=not args.no_map,
