#!/usr/bin/env python3
"""
generate_global_land_comps_table.py
=====================================
Global (non-Singapore) Land Sale Comparables table generator.
Same visual style as generate_land_comps_table.py — navy headers, alternating
fills, subject/comp separation — but without SG-specific columns (Bala
adjustment, leasehold tenure column) and with dynamic currency/unit headers.

Output Schema  (Type hidden)
----------------------------
  Property | Date | Map Marker
  Sale Price ({cur} {unit}) | Site Area ({area_unit}) | Unit Price ({cur}/{area_unit})
  Zoning Type | Location Type | Quality Type | Comment
"""
...
_SCHEMA_BASE = [
    ("Type",                 "type",          "str",   "@",       10),  # hidden (col A)
    ("Property",             "property",      "str",   "@",       36),
    ("Date",                 "launch_date",   "str",   "@",       14),
    ("Map\nMarker",          "map_marker",    "str",   "@",        8),
    ("Sale Price\n(-- --)",  "price_raw",     "float", "#,##0.0", 16),
    ("Site Area\n(--)",      "site_area",     "int",   "#,##0",   13),
    ("Unit Price\n(--/--)",  "unit_price",    "int",   "#,##0",   18),
    ("Zoning Type",          "land_zoning",   "str",   "@",       16),
    ("Location Type",        "location",      "str",   "@",       24),
    ("Quality Type",         "quality",       "str",   "@",       20),
    ("Comment",              "comment",       "str",   "@",       40),
]


def get_global_land_schema(subject_cfg: dict) -> list:
...
def _calc_unit_price(price_raw, price_unit: str, gfa, site_area) -> int | None:
    """Unit Land Price = Sale Price × scale / Max GFA (fall back to site area)."""
    denominator = gfa or site_area
    if not price_raw or not denominator:
        return None
    try:
        scale = _PRICE_SCALE.get(price_unit.upper(), 1_000_000)
        return round(float(price_raw) * scale / float(denominator))
...
def subject_to_row(cfg: dict, subject_cfg: dict) -> dict:
    ...
    site_area  = cfg.get("site_area") or cfg.get("site_area_sf") or cfg.get("site_area_raw")
    max_gfa    = cfg.get("max_gfa") or cfg.get("max_gfa_sf") or cfg.get("gfa_sf")
    return {
        "type":        "Subject",
        "source":      "",
        "property":    cfg.get("property_name", ""),
        "map_marker":  "★",
        "launch_date": cfg.get("launch_date", "") or cfg.get("sale_date", ""),
        "land_zoning": cfg.get("land_zoning", ""),
        "site_area":   int(site_area) if site_area else None,
        "max_gfa":     int(max_gfa) if max_gfa else None,
        "price_raw":   float(price_raw) if price_raw else None,
        "unit_price":  _calc_unit_price(price_raw, price_unit, max_gfa, site_area),
        "location":    cfg.get("location", ""),
        "quality":     cfg.get("quality", ""),
        "comment":     cfg.get("remarks", "") or cfg.get("comment", ""),
    }
...
def comp_to_row(c: dict, subject_cfg: dict) -> dict:
    ...
    site_area  = c.get("site_area") or c.get("site_area_sf") or c.get("site_area_raw")
    max_gfa    = c.get("max_gfa") or c.get("max_gfa_sf") or c.get("gfa_sf")
    unit_price = (c.get("unit_price") or c.get("price_psf_ppr")
                  or _calc_unit_price(price_raw, price_unit, max_gfa, site_area))
    return {
        "type":        "Comparable",
        "source":      _source_label(c.get("_source", "")),
        "property":    name,
        "map_marker":  str(c.get("map_marker", "")),
        "launch_date": str(c.get("launch_date", "") or c.get("sale_date", "")),
        ...
        "comment":     str(c.get("remarks", "") or c.get("comment", "")),
    }
...
def _avg_row(ws, row: int, first_r: int, last_r: int, schema: list):
    avg_keys = {"price_raw", "site_area", "max_gfa", "unit_price"}
    for col, entry in enumerate(schema, 1):
        key, dtype, fmt = entry[1], entry[2], entry[3]
...
def build_workbook(subject_row: dict, comp_rows: list,
    ...
    schema    = get_global_land_schema(subject_cfg)
    cur       = get_deal_currency(subject_cfg)
    unit      = get_deal_price_unit(subject_cfg)
    area_unit = get_deal_area_unit(subject_cfg)
    nc        = len(schema)
    ...
    t = ws.cell(row=r, column=1,
                value=f"{deal_name.upper()}  —  LAND SALE COMPARABLES")
    ...
    r = 4
    _section_header(ws, r, "  Subject Site — Based on Underwriting", nc)
    r = 5; _col_headers(ws, r, schema)
    r = 6; _data_row(ws, r, subject_row, schema, bold=True, is_subject=True)

    ws.row_dimensions[7].height = 6

    # ── Table 2: Comparable Land Sales ───────────────────────────────────────
    r = 8
    _section_header(ws, r, "  Comparable Land Sales / Government Land Tenders", nc)
    ...
    notes = (
        ...
        f"(3) Unit Land Price ({cur}/{area_unit}) = Sale Price × scale ÷ Max GFA "
        f"(or Site Area if Max GFA unavailable; calculated if not provided in source).  "
